...
def plot_change_detection_map(pre_v, tar_v, H, W, threshold=0.1):
    """
    根据预测结果（pre_v）和真实标签（tar_v）绘制变化检测图

    :param pre_v: 预测值
    :param tar_v: 真实标签
    :param H: 图像的高度
    :param W: 图像的宽度
    :param threshold: 变化检测的阈值，决定了哪些区域的变化被视为显著
    :return: 变化检测图
    """
    # 将预测值和真实标签重塑为图像形状
    #pre_v = pre_v.reshape(H, W)
    #tar_v = tar_v.reshape(H, W)

    # 计算变化：使用预测值与真实标签之间的差异
    change_map = np.abs(pre_v - tar_v) > threshold  # 如果差异超过阈值，认为发生了变化


    # 绘制变化检测图
    plt.figure(figsize=(8, 6))
    plt.imshow(change_map, cmap='jet', interpolation='nearest')  # 使用 jet colormap
    plt.colorbar(label='Change Intensity')
    plt.title('Change Detection Map')
    #plt.xlabel('Pixel Index')
    #plt.ylabel('Pixel Index')
    plt.savefig('/home5/wsq/HyperSIGMA/ChangeDetection/CD_Result/SpatSIGMA_CD/f_tar.png', format='png', dpi=300, bbox_inches='tight')
    plt.show()

    return change_map

def test_model(args, model_save_PATH, result_save_file,plt_save):
    ...
    # 绘制变化检测图
    change_map = plot_change_detection_map(binary_map, binary_label, H, W, threshold=0.1)

    # 绘制变化检测图
    plt.figure(figsize=(8, 6))
    #plt.imshow(binary_map, cmap='jet', interpolation='nearest')  # 使用 jet colormap
    plt.imshow(binary_map, cmap='binary', interpolation='nearest')  # 使用 jet colormap
    plt.colorbar(label='Change Intensity')
    plt.title('Change Detection Pre Map')
    # plt.xlabel('Pixel Index')
    # plt.ylabel('Pixel Index')
    plt.savefig('/home5/wsq/HyperSIGMA/ChangeDetection/CD_Result/SpatSIGMA_CD/f_tar.png', format='png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    # 绘制变化检测图
    plt.figure(figsize=(8, 6))
    plt.imshow(binary_label, cmap='binary', interpolation='nearest')  # 使用 jet colormap
    plt.colorbar(label='Change Intensity')
    plt.title('Change Detection tar Map')
    # plt.xlabel('Pixel Index')
    # plt.ylabel('Pixel Index')
    plt.savefig('/home5/wsq/HyperSIGMA/ChangeDetection/CD_Result/SpatSIGMA_CD/f_tar.png', format='png', dpi=300, bbox_inches='tight')
    plt.show()

    return bmap, oa_kappa,change_map
...
def run(Dataset_name, seed, mode='Spat_Spec_Pretraining',use_checkpoint=True):
    ...
    plt_save=path + save_name + '.png'
    bmap, oa_kappa,change_map = test_model(args, model_save_PATH, result_save_file,plt_save)
...
if __name__ == '__main__':
    ...
    # 'spat-base.pth' (https://pan.baidu.com/s/1kShixCeWhPGde-vLLxQLJg?pwd=vruc)
    # 'spec-base.pth' (https://pan.baidu.com/s/1VinBf4qnN98aa6z7TZ-ENQ?pwd=mi2y)
    ...
    Dataset_name = 'Farmland' #  ['Hermiston', 'Farmland', 'Barbara', 'BayArea']
