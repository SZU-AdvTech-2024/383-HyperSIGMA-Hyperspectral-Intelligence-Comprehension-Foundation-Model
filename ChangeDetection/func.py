...
def get_args(dataset,seed_i,use_checkpoint=True,mode='Spat_Spec_Pretraining'):
    ...
    if mode =='Spat_Pretraining':
        ...
        if use_checkpoint:
            if os.path.isfile(path+'spat-base.pth'):
                pretrain_Spatpath = path + 'spat-base.pth'
            else:
                print('Please download the pretrained model checkpoint firstly at '
                      'https://pan.baidu.com/s/1kShixCeWhPGde-vLLxQLJg?pwd=vruc')
                raise ValueError(path + 'spat-base.pth is not exit')

            if os.path.isfile(path+'spec-base.pth'):
                pretrain_Specpath = path + 'spec-base.pth'
            else:
                print('Please download the pretrained model checkpoint firstly at '
                      'https://pan.baidu.com/s/1VinBf4qnN98aa6z7TZ-ENQ?pwd=mi2y')
                raise ValueError( path + 'spec-base.pth is not exit')
...
def get_trainSmp_Position(dataset):
    if dataset == 'BayArea':
        print('----------------------input data:   Bay ------------------------')
        data_t1 = loadmat(r'/home5/wsq/HyperSIGMA/ChangeDetection/data/BayArea.mat')['T1']
        data_t2 = loadmat(r'/home5/wsq/HyperSIGMA/ChangeDetection/data/BayArea.mat')['T2']
        data_label = loadmat(r'/home5/wsq/HyperSIGMA/ChangeDetection/data/BayArea.mat')['GT']
        uc_position = np.array(np.where(data_label == 2)).transpose(1, 0)
        c_position = np.array(np.where(data_label == 1)).transpose(1, 0)
        print((uc_position.shape[0], c_position.shape[0]))
    elif dataset == 'Barbara':
        print('----------------------input data:   Barbara ------------------------')
        data_t1 = loadmat(r'/home5/wsq/HyperSIGMA/ChangeDetection/data/Barbara.mat')['T1']
        data_t2 = loadmat(r'/home5/wsq/HyperSIGMA/ChangeDetection/data/Barbara.mat')['T2']
        data_label = loadmat(r'/home5/wsq/HyperSIGMA/ChangeDetection/data/Barbara.mat')['GT']
        uc_position = np.array(np.where(data_label == 2)).transpose(1, 0)
        c_position = np.array(np.where(data_label == 1)).transpose(1, 0)
        print((uc_position.shape[0], c_position.shape[0]))
    elif dataset == 'Hermiston':
        data_t1 = loadmat(r'/home5/wsq/HyperSIGMA/ChangeDetection/data/Sa1.mat')['T1']
        data_t2 = loadmat(r'/home5/wsq/HyperSIGMA/ChangeDetection/data/Sa2.mat')['T2']
        data_label = loadmat(r'/home5/wsq/HyperSIGMA/ChangeDetection/data/SaGT.mat')['GT']
        uc_position = np.array(np.where(data_label == 2)).transpose(1, 0)
        c_position = np.array(np.where(data_label == 1)).transpose(1, 0)
        print((uc_position.shape[0], c_position.shape[0]))
    elif dataset == 'Farmland':
        data_t1 = sio.loadmat(r'/home5/wsq/HyperSIGMA/ChangeDetection/data/Farm1.mat')['imgh']  # 450*140
        data_t2 = sio.loadmat(r'/home5/wsq/HyperSIGMA/ChangeDetection/data/Farm2.mat')['imghl']
        data_label = sio.loadmat(r'/home5/wsq/HyperSIGMA/ChangeDetection/data/GTChina1.mat')['label']
        uc_position = np.array(np.where(data_label == 1)).transpose(1, 0)
        c_position = np.array(np.where(data_label == 2)).transpose(1, 0)
        print((uc_position.shape[0], c_position.shape[0]))
        data_label[data_label == 0] = 2
