...
from func import ts_generation
...
def train(args, epoch, net, optimizer, scheduler, trn_loader, criterion):
    ...
    for idx, (X_data, y_target, ts_vector) in enumerate(trn_loader):

        X_data=Variable(X_data.float()).cuda(non_blocking=True)
        ts_vector = Variable(ts_vector.float()).cuda(non_blocking=True)
        y_target = Variable(y_target.long()).cuda(non_blocking=True)

        pred_prob = net.forward(X_data, ts_vector)

        y_pred_prob = torch.stack((1-pred_prob, pred_prob), 1)

        #y_pred = y_pred_prob > 0

        loss = criterion(y_pred_prob, y_target)
...
def validation(args, epoch, net, val_loader):
    ...
    for idx, (X_data, y_target, ts_vector) in enumerate(val_loader):
        with torch.no_grad():

            X_data = Variable(X_data.float()).cuda(non_blocking=True)
            ts_vector = Variable(ts_vector.float()).cuda(non_blocking=True)
            y_target = Variable(y_target.float().long()).cuda(non_blocking=True)
            y_pred_prob = net.forward(X_data, ts_vector)

            y_pred_prob = standard(y_pred_prob.cpu().numpy()),
...
## dataset setting
parser.add_argument('--dataset', type=str, default='mosaic',
                    choices=['mosaic','aviris'],
                    help='dataset name')
...
save_path = '/home7/wsq/HyperSIGMA/HyperspectralDetection/Target_Detevtion/work_dir/vitseg_htd/{}'.format(args.mode)
...
target_spectrum = ts_generation(data_tube, gt_tube, 0)
...
print('Implementing HTD seg model')
...
for count in range(0, args.experiment_num):
    ...
    if args.input_mode == 'whole':
        ...
        ts_vector = target_spectrum.reshape(1, d)
        ...
        ts_vector = np.tile(target_spectrum.reshape(1, d), (N,1))
    ...
    ts_vector = torch.from_numpy(ts_vector)
    ...
    trn_dataset = TensorDataset(X_data, y_trn_data, ts_vector)
    trn_loader=DataLoader(trn_dataset,batch_size=args.batch_size,num_workers=args.workers,
                        shuffle=True, drop_last=True, pin_memory=True)


    val_dataset = TensorDataset(X_data, y_val_data, ts_vector)
    val_loader = DataLoader(val_dataset, batch_size=args.val_batch_size,shuffle=False, pin_memory=True)


    from models.models import SpatialHTDFramework, SSHTDFramework
    if args.mode == 'sa':
        net = SpatialHTDFramework(args, img_size=img_size, in_channels = X_data.shape[1])
    elif args.mode == 'ss':    
        net = SSHTDFramework(args, img_size=img_size, in_channels = X_data.shape[1])
    ...
    for i in range(0, args.epochs):
        ...
        if (i+1) % args.val_freq==0:
            ...
            if val_AUC >= best_AUC:
                filename = os.path.join(save_path, 'HTD_' + str(FLAG) + '_valbest_tmp' + '.pth')
    ...
    if args.input_mode == 'whole':
        ...
        with torch.no_grad():
            ...
            pred = net(X_data.float(), ts_vector.float())
            y_tes_pred = pred.cpu().numpy()
        ...
        for j in range(len(Ly)):
            for k in range(len(Lx)):
                ...
                with torch.no_grad():
                    pred = net(img_part.float(), ts_vector[[0],:].float())

                part_pred = pred[0].cpu().numpy()

                part_pred_map = part_pred > 0
    ...
    if auc1 >= best_auc or count==0:
        if args.resume:
            torch.save(net, os.path.join(save_path, 'HTD_' + str(FLAG) + '_ft.pth'))
            best_auc = auc1
            np.save(os.path.join(save_path, 'HTD_' + str(FLAG) + '_pre_map_ft.npy'), y_tes_pred_map)
        else:
            torch.save(net, os.path.join(save_path, 'HTD_'+str(FLAG) +'.pth'))
            best_auc = auc1
            np.save(os.path.join(save_path, 'HTD_'+str(FLAG)+'_pre_map.npy'), y_tes_pred_map)
...
if args.resume:
    scio.savemat(os.path.join(save_path, 'HTD_'+ str(FLAG)+ '_ft.mat'), {'data': Experiment_result})
    y_disp_all = np.load(os.path.join(save_path, 'HTD_'+ str(FLAG) +'_pre_map_ft.npy'))*255
    cv2.imwrite(os.path.join(save_path, 'HTD_'+ str(FLAG)+'_segmap_ft.png'), y_disp_all.reshape(r, c))
    np.save(os.path.join(save_path, 'HTD_' + str(FLAG) + '_prob_map_ft.npy'), y_tes_pred)
else:
    scio.savemat(os.path.join(save_path, 'HTD_'+str(FLAG)+'.mat'),{'data':Experiment_result})
    y_disp_all = np.load(os.path.join(save_path, 'HTD_'+str(FLAG)+'_pre_map.npy'))*255
    cv2.imwrite(os.path.join(save_path, 'HTD_'+str(FLAG)+'_segmap.png'), y_disp_all.reshape(r,c))
    np.save(os.path.join(save_path, 'HTD_' + str(FLAG) + '_prob_map.npy'), y_tes_pred)
