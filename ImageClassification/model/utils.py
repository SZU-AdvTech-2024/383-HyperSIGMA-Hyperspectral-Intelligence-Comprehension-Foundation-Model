...
#重新计算图像的行数 (num_H) 和列数 (num_W)，确保图像现在能被补丁大小整除
    #当图像高度不能被 img_size 完整整除时，sub_H 就表示需要额外处理的像素数，以确保划分为多个大小一致的补丁
    #在 Get_train_and_test_data 函数中，如果 sub_H 不等于零，意味着需要对图像进行镜像填充，使其高度可以被 img_size 整除。这是为了在后续提取补丁时避免剩余的部分像素无法被完全包含的问题
    num_H = H // img_size
    num_W = W // img_size
    sub_H = H % img_size
    sub_W = W % img_size
    ...
    #如果图像宽度不能被补丁大小整除，则使用镜像填充来使图像宽度适应完整的补丁大小：
#gap 计算需要填充的宽度。
#使用图像右侧的部分进行镜像填充。
    if sub_H != 0:
